...
class OnPremiseCloudBuilder extends ICloudBuilder {
  constructor(factory) {
    super();
  ...
  setFlavor(choice){ 
    this.vmSpec.flavor = choice;
    try {
      const pd = require('../core/provisioningDirector');
      const types = pd.MACHINE_TYPES.ONPREM || {};
  ...
  async buildVM(){ 
    ...
    try {
      const pd = require('../core/provisioningDirector');
      const choice = this.vmSpec.flavor || this.vmSpec.instancia || this.vmSpec.machine_type || this.vmSpec.tamaño_maquina;
      if (choice) {
        const types = pd.MACHINE_TYPES.ONPREM || {};
        for (const family of Object.keys(types)) {
          const map = types[family];
  ...
  cloneConfiguration(){ 
    const clone = new OnPremiseCloudBuilder(this.factory);
...
module.exports = OnPremiseCloudBuilder;
